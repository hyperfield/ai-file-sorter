...
struct TestEnvironment {
    ...
    EnvVarGuard extension_state_guard;
    Settings settings;
    ...
    TestEnvironment()
        : home_dir(),
          ...
          extension_state_guard("AI_FILE_SORTER_TEST_EXPLORER_EXTENSION_STATE",
...
TEST_CASE("Installed Explorer extension suppresses support prompts") {
    TestEnvironment env;
    EnvVarGuard extension_state_guard("AI_FILE_SORTER_TEST_EXPLORER_EXTENSION_STATE",
    ...
    CHECK(callback_count == 0);
    CHECK_FALSE(SupportCodeManager(std::filesystem::path(env.settings.get_config_dir()))
                    .is_prompt_permanently_disabled());
    CHECK(env.settings.get_next_support_prompt_threshold() == threshold);
}
