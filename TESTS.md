...
Covered cases include categorization with and without subcategories, whitelist-restricted categorization, selected-file auto-apply boundaries, document renaming in English/French/Simplified Chinese/Hindi, optional image-content renaming in the same languages when visual artifacts are configured, WAV metadata rename, and categorize-and-rename review-plan generation. Exact model labels are not treated as golden output; assertions focus on stable invariants such as non-empty categories, whitelist confinement, extension preservation, localized script signals, and review-plan creation.
