...
std::string DatabaseManager::normalize_label(const std::string &input) const {
    return DatabaseTaxonomyNormalizer::normalize_label(input);
}
...
double DatabaseManager::string_similarity(const std::string &a, const std::string &b) {
    return DatabaseTaxonomyNormalizer::string_similarity(a, b);
}
...
DatabaseManager::resolve_category(const std::string &category,
    ...
    const auto normalized = DatabaseTaxonomyNormalizer::normalize_category(category, subcategory);
    std::string key = make_key(normalized.normalized_category, normalized.match_subcategory);

    int taxonomy_id = resolve_existing_taxonomy(key,
                                                normalized.normalized_category,
                                                normalized.match_subcategory);
    if (taxonomy_id == -1 &&
        normalized.match_subcategory != normalized.normalized_subcategory) {
        const std::string raw_key = make_key(normalized.normalized_category,
                                             normalized.normalized_subcategory);
        taxonomy_id = resolve_existing_taxonomy(raw_key,
                                                normalized.normalized_category,
                                                normalized.normalized_subcategory);
    }
...
DatabaseManager::resolve_category_for_language(const std::string& category,
    ...
    const std::string stripped_subcategory =
        DatabaseTaxonomyNormalizer::strip_trailing_stopwords(normalized_subcategory);
