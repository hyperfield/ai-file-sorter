...
std::optional<std::filesystem::path> sanitize_archive_path(const std::string& entry_name)
{
    const std::filesystem::path normalized =
        std::filesystem::path(entry_name).lexically_normal();
...
PluginArchiveExtractor::ExtractionResult
PluginArchiveExtractor::ExtractionResult::success(std::filesystem::path root,
...
PluginArchiveExtractor::ExtractionResult
PluginArchiveExtractor::ExtractionResult::failure(std::string error)
{
    return ExtractionResult{{}, {}, std::move(error)};
}

bool PluginArchiveExtractor::ExtractionResult::ok() const
{
    return !extracted_root.empty() && !manifest_path.empty();
}

bool PluginArchiveExtractor::supports_archive(const std::filesystem::path& package_path)
{
    const auto extension = ascii_lower_copy(package_path.extension().string());
    return extension == ".aifsplugin" || extension == ".zip";
}

PluginArchiveExtractor::ExtractionResult PluginArchiveExtractor::extract_archive(
    ...
    const std::filesystem::path& destination_root)
{
    ...
    if (!archive) {
        ...
        const std::string error_message =
            message ? std::string(message) : std::string("Failed to open plugin archive.");
    ...
    for (zip_uint64_t index = 0; index < static_cast<zip_uint64_t>(entry_count); ++index) {
        ...
        if (is_directory_entry(entry_name)) {
            ...
            if (ec) {
                ...
                return ExtractionResult::failure(
                    "Failed to create extracted plugin directory: " + ec.message());
            }
            continue;
        }
        ...
        if (sanitized->filename() == "manifest.json") {
            if (manifest_path.has_value()) {
                zip_close(archive);
                return ExtractionResult::failure(
                    "The plugin archive contains multiple manifest.json files.");
            }
