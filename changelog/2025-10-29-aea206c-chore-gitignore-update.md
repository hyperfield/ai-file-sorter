# 2025-10-29: chore(gitignore): update

## Covered commits
- `aea206c` `2025-10-29` `chore(gitignore): update`
...
Before this commit, the repository reflected the state immediately preceding `aea206c`. After this commit, the files listed above incorporated the exact delta shown in the excerpt and file list, which is why this entry is stored as its own chapter instead of being folded into a broader release note.
...
+create_dmg.sh
 ...
 # References
 # includePaths.txt
@@ -106,5 +107,6 @@ app/include/llama/*.h
 # Build artifacts
