# 2026-03-08: chore(deps): rewind llama.cpp and Catch2 submodules

## Covered commits
- `87ddf90` `2026-03-08` `chore(deps): rewind llama.cpp and Catch2 submodules`
...
Before this commit, the repository reflected the state immediately preceding `87ddf90`. After this commit, the files listed above incorporated the exact delta shown in the excerpt and file list, which is why this entry is stored as its own chapter instead of being folded into a broader release note.
...
The excerpt is taken from the commit diff for `chore(deps): rewind llama.cpp and Catch2 submodules`. The most relevant surfaces are `app/include/external/llama.cpp`, `external/Catch2`; those lines show where the repository crossed from the previous implementation or documentation state into the new one.
