# 2025-08-10: fix(readme): return accidentally lost changes in text

## Covered commits
- `ccf98ce` `2025-08-10` `fix(readme): return accidentally lost changes in text`
...
Before this commit, the repository reflected the state immediately preceding `ccf98ce`. After this commit, the files listed above incorporated the exact delta shown in the excerpt and file list, which is why this entry is stored as its own chapter instead of being folded into a broader release note.
...
The excerpt is taken from the commit diff for `fix(readme): return accidentally lost changes in text`. The most relevant surfaces are `README.md`; those lines show where the repository crossed from the previous implementation or documentation state into the new one.
