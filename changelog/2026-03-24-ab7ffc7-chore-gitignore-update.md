# 2026-03-24: chore(gitignore): update

## Covered commits
- `ab7ffc7` `2026-03-24` `chore(gitignore): update`
...
Before this commit, the repository reflected the state immediately preceding `ab7ffc7`. After this commit, the files listed above incorporated the exact delta shown in the excerpt and file list, which is why this entry is stored as its own chapter instead of being folded into a broader release note.
...
--- a/.gitignore
+++ b/.gitignore
@@ -133,6 +133,8 @@ app/include/llama/*.h
 /models/
 /rd/
 /prototypes/
+/store/ms-store/
