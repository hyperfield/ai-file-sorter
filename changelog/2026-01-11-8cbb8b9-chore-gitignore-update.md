# 2026-01-11: chore(gitignore): update

## Covered commits
- `8cbb8b9` `2026-01-11` `chore(gitignore): update`
...
Before this commit, the repository reflected the state immediately preceding `8cbb8b9`. After this commit, the files listed above incorporated the exact delta shown in the excerpt and file list, which is why this entry is stored as its own chapter instead of being folded into a broader release note.
...
--- a/.gitignore
+++ b/.gitignore
@@ -55,6 +55,14 @@ create_macos_bundle.sh
 create_dmg.sh
 package_deb.sh
