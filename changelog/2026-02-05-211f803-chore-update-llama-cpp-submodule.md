# 2026-02-05: chore: update llama.cpp submodule

## Covered commits
- `211f803` `2026-02-05` `chore: update llama.cpp submodule`
...
Before this commit, the repository reflected the state immediately preceding `211f803`. After this commit, the files listed above incorporated the exact delta shown in the excerpt and file list, which is why this entry is stored as its own chapter instead of being folded into a broader release note.
...
The excerpt is taken from the commit diff for `chore: update llama.cpp submodule`. The most relevant surfaces are `app/include/external/llama.cpp`; those lines show where the repository crossed from the previous implementation or documentation state into the new one.
