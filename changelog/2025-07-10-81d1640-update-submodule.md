# 2025-07-10: Update submodule

## Covered commits
- `81d1640` `2025-07-10` `Update submodule`
...
Before this commit, the repository reflected the state immediately preceding `81d1640`. After this commit, the files listed above incorporated the exact delta shown in the excerpt and file list, which is why this entry is stored as its own chapter instead of being folded into a broader release note.
...
The excerpt is taken from the commit diff for `Update submodule`. The most relevant surfaces are `app/include/external/llama.cpp`; those lines show where the repository crossed from the previous implementation or documentation state into the new one.
